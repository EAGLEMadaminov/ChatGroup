import { useContext, useEffect, useState } from "react";
import { useForm } from "react-hook-form";
import axios from "axios";
import { SideContext } from "../Context/SideBarContext";
import { Link } from "react-router-dom";
import { toast } from "react-toastify";
import { SearchContext } from "../Context/SearchContext";

const SideBar = () => {
  const [showCreateBtn, setShowCreateBtn] = useState(false);
  const [showCreateModal, setShowCreateModal] = useState(false);
  const { setGroup, isLeaveGroup } = useContext(SideContext);
  const { isJoindedGroup } = useContext(SearchContext);
  const [getInfo, setGetInfo] = useState({});
  const [isCreateGroup, setIsCreateGroup] = useState(false);

  const { register, handleSubmit } = useForm();

  let token = localStorage.getItem("token");

  const createGroup = async (data) => {
    try {
      let { data: groupInfo } = await axios.post("/groups", data, {
        headers: {
          "x-auth-token": token,
        },
      });
      setIsCreateGroup(true);
      setGroup(groupInfo);
    } catch (error) {
      toast.error(error.response.data.message);
    }
    setShowCreateModal(false);
  };

  useEffect(() => {
    async function getMyGroupsFunc() {
      try {
        let { data: mygroup } = await axios.get("/groups", {
          headers: {
            "x-auth-token": token,
          },
        });
        setGetInfo(mygroup);
      } catch (error) {
        toast.error(error.response.data.message);
      }
    }
    getMyGroupsFunc();
  }, [token, isLeaveGroup, isJoindedGroup, isCreateGroup]);

  return (
    <div className="pt-[70px] px-5 w-[250px]">
      <ul>
        <li>
          <Link
            to="/main"
            className="flex items-center my-4  gap-2 hover:pl-5 hover:bg-[#d3d4d5] p-2 rounded-lg hover:border"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="16"
              width="14"
              viewBox="0 0 448 512"
            >
              <path
                fill="#0d6efd"
                d="M224 256A128 128 0 1 0 224 0a128 128 0 1 0 0 256zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512H418.3c16.4 0 29.7-13.3 29.7-29.7C448 383.8 368.2 304 269.7 304H178.3z"
              />
            </svg>{" "}
            Profile
          </Link>
        </li>
        <li className=" ">
          <button
            className="flex hover:pl-5 items-center gap-2 hover:bg-[#d3d4d5] p-2 rounded-lg w-full"
            onClick={() => setShowCreateBtn(!showCreateBtn)}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="16"
              width="20"
              viewBox="0 0 640 512"
            >
              <path
                fill="#0d6efd"
                d="M88.2 309.1c9.8-18.3 6.8-40.8-7.5-55.8C59.4 230.9 48 204 48 176c0-63.5 63.8-128 160-128s160 64.5 160 128s-63.8 128-160 128c-13.1 0-25.8-1.3-37.8-3.6c-10.4-2-21.2-.6-30.7 4.2c-4.1 2.1-8.3 4.1-12.6 6c-16 7.2-32.9 13.5-49.9 18c2.8-4.6 5.4-9.1 7.9-13.6c1.1-1.9 2.2-3.9 3.2-5.9zM0 176c0 41.8 17.2 80.1 45.9 110.3c-.9 1.7-1.9 3.5-2.8 5.1c-10.3 18.4-22.3 36.5-36.6 52.1c-6.6 7-8.3 17.2-4.6 25.9C5.8 378.3 14.4 384 24 384c43 0 86.5-13.3 122.7-29.7c4.8-2.2 9.6-4.5 14.2-6.8c15.1 3 30.9 4.5 47.1 4.5c114.9 0 208-78.8 208-176S322.9 0 208 0S0 78.8 0 176zM432 480c16.2 0 31.9-1.6 47.1-4.5c4.6 2.3 9.4 4.6 14.2 6.8C529.5 498.7 573 512 616 512c9.6 0 18.2-5.7 22-14.5c3.8-8.8 2-19-4.6-25.9c-14.2-15.6-26.2-33.7-36.6-52.1c-.9-1.7-1.9-3.4-2.8-5.1C622.8 384.1 640 345.8 640 304c0-94.4-87.9-171.5-198.2-175.8c4.1 15.2 6.2 31.2 6.2 47.8l0 .6c87.2 6.7 144 67.5 144 127.4c0 28-11.4 54.9-32.7 77.2c-14.3 15-17.3 37.6-7.5 55.8c1.1 2 2.2 4 3.2 5.9c2.5 4.5 5.2 9 7.9 13.6c-17-4.5-33.9-10.7-49.9-18c-4.3-1.9-8.5-3.9-12.6-6c-9.5-4.8-20.3-6.2-30.7-4.2c-12.1 2.4-24.7 3.6-37.8 3.6c-61.7 0-110-26.5-136.8-62.3c-16 5.4-32.8 9.4-50 11.8C279 439.8 350 480 432 480z"
              />
            </svg>
            Groups
          </button>
        </li>
      </ul>
      {showCreateBtn ? (
        <div>
          <button
            onClick={() => setShowCreateModal(!showCreateModal)}
            className="ml-5 my-4 w-full flex items-center gap-2 hover:pl-5 hover:bg-[#d3d4d5] p-2 rounded-lg"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="16"
              width="14"
              viewBox="0 0 448 512"
            >
              <path
                fill="#0d6efd"
                d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z"
              />
            </svg>
            Create Group
          </button>
          {getInfo.length > 0 ? (
            <div>
              {getInfo.map((item) => {
                return (
                  <Link
                    key={item.id}
                    to={`/groups/${item._id}`}
                    className="text-left ml-7 block w-full mb-3 hover:pl-5 hover:bg-[#d3d4d5] p-2 rounded-lg"
                  >
                    {item.name}
                  </Link>
                );
              })}
            </div>
          ) : (
            ""
          )}
        </div>
      ) : (
        ""
      )}
      {showCreateModal ? (
        <div className="absolute top-[120px] z-[2]  left-[215px] bg-white border rounded-lg">
          <div className="flex py-2 bg-[#E9ECEF] justify-center rounded-t-lg items-center">
            <h2 className="font-semibold">Group name and password</h2>
            <button
              className="ml-1 text-gray-500 text-xl"
              onClick={() => setShowCreateModal(false)}
            >
              ✖
            </button>
          </div>
          <form
            className="flex flex-col gap-2 px-5 my-4 "
            onSubmit={handleSubmit(createGroup)}
          >
            <input
              type="text"
              placeholder="Group name"
              className="p-2 rounded-lg border outline-none "
              {...register("name")}
            />
            <input
              type="password"
              placeholder="Group password"
              className="p-2 rounded-lg border outline-none "
              {...register("password")}
            />
            <div className="flex gap-2 justify-between">
              <button className="p-2 px-3 w-full bg-[#0D6EFD] rounded-lg text-white">
                Create
              </button>
              <button className="p-1 text-[#0D6EFD] px-3 border w-full rounded-lg border-[#0D6EFD]">
                Cansel
              </button>
            </div>
          </form>
        </div>
      ) : (
        ""
      )}
    </div>
  );
};

export default SideBar;
